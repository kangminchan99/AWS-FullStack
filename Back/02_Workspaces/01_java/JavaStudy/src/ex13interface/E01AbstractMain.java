...
- 추상클래스는 대부분 추상 메서드를 포함하고 있으므로, 이를 상속 받는 클래스는
반드시 추상 메서드를 오버라이딩 해야한다.
...
/*
추상 클래스2: 추상메서드를 포함한 클래스. 추상메서드를 멤버로 포함한 클래스는
반드시 abstract로 선언해야한다. 그렇지 않으면 에러가 발생한다.(java의 규칙)
 */
...
추상 메서드를 자식 쪽에서 오버라이딩을 필수로 해야한다.
 */
...
//        AbsClass1 absClass1 = new AbsClass1();
        ...
        // 부모 타입의 참조변수로 자식 인스턴스를 참조한다.
        AbsClass2 absClass2 = new AbsClassChild2();

        // 부모 쪽의 멤버 메서드를 호출할 수 있다. 하지만 오버라이딩 되었으므로
        // 자식쪽의 멤버 메서드가 동작하게 된다.
        absClass2.absMethod(100);

        /*
        하지만 부모 타입의 참조변수로 자식영역에 접근은 불가능하므로,
        접근을 위해 자식 타입으로 다운 캐스팅 해야한다.
         */
        ((AbsClassChild2)absClass2).newMethod();

        /*
        추상 클래스내에 선언된 정적 메서드는 오버라이딩의 대상이 될 수 없으므로
        static의 기본 규칙을 따른다. 인스턴스 생성 없이 단지 클래스명으로
        접근하여 호출할 수 있다.
         */
        AbsClass1.staticMethod();
        AbsClassChild1.staticMethod();
        System.out.println("int형의 최댓값:" + AbsClass1.MAX_INT);
    }
}
